...
def dijkstra_logic(creature_coord, goal_coord, region_map, list_solid_object):
    """
    :param creature_coord: coordinates of creature for whom we are looking for a way
    ...
    start = (creature_coord[0], creature_coord[1])
...
class Creature(SolidObject):
    ...
    def pathfinder(self, goal_coord, region_map, list_solid_object):
        ...
        :return: list[list[float, float],...] - list of tiles to go through
        """
        path = dijkstra_logic([int(self.coord[0]), int(self.coord[1])], goal_coord, region_map, list_solid_object)
        return path
